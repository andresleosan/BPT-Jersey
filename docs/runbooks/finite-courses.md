...
Office subject export is paginated at 30, scoped by applicant UID, omits storage keys and signed URLs, and omits minor candidate identity from an adult export. The optional studentId mode returns only the selected participant’s name, date of birth, course identifier and enrolment status, with a maximum of 30 rows. Every page checks current canonical self/guardian access for subjectUid and live office authority in the same transaction. It excludes payer details, other participants, evidence and financial fields. The response returns only to the requesting office session; no email, external transfer or automatic file delivery occurs. Financial retention still requires office review. It is not a self-service destructive erasure endpoint. No proposed ADR-008 retention policy is enabled.
