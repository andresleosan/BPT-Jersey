...
| Collection               | Required fields                                                                                                                                                                                                                                                 | References                                                                                                                                                          | Classification                                                                                                                                                                       | Write authority                                                                                                                                                                            | Deletion/history rule                                                                                                                                                                                                                   |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `locations`              | `locationId`, `academyId`, `name`, `address`, `timezone`, `active`, optional `geofence` (`latitude`, `longitude` of the academy site, T109), `schemaVersion`, `createdAt`, `createdBy`, `updatedAt`, `updatedBy`, `status`                                      | No mandatory document reference; sessions and classes reference `locationId`.                                                                                       | `Internal`; an intentionally published address projection may be `Public`. `geofence` is the premises, never a person, and is returned to every authenticated role with the catalog. | Academy configuration backend; `geofence` is written only by `saveLocationGeofence` (owner/administrator, audited `location.geofence.saved`); operational values remain subject to `T008`. | Deactivate rather than delete when referenced by classes, sessions, attendance, or audit. Preserve historical location references.                                                                                                      |
...
| Collection        | Required fields                                                                                                                                                                                                                                                                                                                                                         | References                                                                                                                         | Classification                                                                                                                                                                     | Write authority                                                                                                                                                                                                            | Deletion/history rule                                                                                                                                                                                                                     |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `attendance`      | `attendanceId`, `academyId`, `sessionId`, `studentId`, `state`, `occurredAt`, `correctionOf`, optional `proximity` (`signal` within/outside/unavailable, `distanceMeters`, `accuracyMeters`, `overrideReason`; check-ins only, T109), `schemaVersion`, `createdAt`, `createdBy`, `updatedAt`, `updatedBy`, `status`                                                     | `sessionId` -> `sessions`, `studentId` -> `students`, `correctionOf` -> the canonical attendance record when this is a correction. | `Confidential`; `proximity` describes the staff device and carries staff free text, so client reads omit it. No member coordinate is ever stored: only a distance and an accuracy. | Attendance backend; check-in methods and the proximity measurement are inputs, not authority. An `outside` signal requires a staff reason and writes `attendance.proximity_override` to `auditEvents`.                     | The canonical record uses the deterministic ID. A correction stays in `attendance` with a backend-generated opaque ID, points to the canonical record through `correctionOf`, preserves the original, and writes an `auditEvents` record. |
| `noShowPenalties` | `penaltyId` (deterministic `sessionId__studentId`), `academyId`, `sessionId`, `studentId`, `locationId`, `amountMinor` (1500), `currency` (`GBP`), `status` (`proposed`/`charged`/`waived`), `sessionStartAt`, `proposedAt`, `proposedBy`, `resolution` (`decision`, `reason`, `invoiceId`, `resolvedAt`, `resolvedBy`) or null, `schemaVersion`, timestamps and actors | `sessionId` -> `sessions`, `studentId` -> `students`, `resolution.invoiceId` -> the manual invoice office issued in `invoices`.    | `Confidential`                                                                                                                                                                     | Penalty backend (T111). Staff propose from the session's canonical attendance; only office resolves, once, and both steps write `auditEvents`. A Town no-show covered by an approved medical leave never earns a proposal. | Resolution is terminal and preserved; nothing is charged automatically, so a waiver or a charge is always an explicit audited human decision.                                                                                             |
