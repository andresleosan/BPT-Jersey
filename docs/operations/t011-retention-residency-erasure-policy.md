...
| IP del actor en cualquier evento de auditoría (`actorIp`) | Escritura del evento | 12 meses — **implementado**, no propuesta | Barrido diario `sweepClassIpRetention` borra el campo `actorIp` de **todo** evento de auditoría con más de 12 meses, sin filtrar por acción, y conserva el evento intacto; hoy solo los eventos de clases escriben ese campo, pero el barrido no lo comprueba, así que cualquier acción futura que llegue a guardar una IP quedará sujeta al mismo plazo de doce meses sin que nadie lo decida explícitamente | Baja: es el único plazo de esta tabla con barrido automático en producción y prueba (`apps/functions/src/audit/class-ip-retention-sweep.test.ts`) |
