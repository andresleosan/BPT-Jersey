...
const RESOLUTION_REQUIREMENTS = {
  ...
  T108: [
    "ESTADO al 2026-09-09: dieciseis rebanadas entregadas y el camino forward completo -dry-run, runner, verificacion y cutover- ensayado de punta a punta contra un Firestore real. Van dos ejecutores de siete. El detalle de cada rebanada esta en tasks.md; lo que queda abajo es lo que deshace o repara una migracion, mas la autorizacion que la arranca.",
    ...
    "El registro de cierre de referencias, member-directory-reference-closure-v1, que la compensacion necesita antes de existir. AUDITADO 2026-09-09 antes de escribirlo, y la auditoria cambio el plan: TENANT_BACKUP_V3_DIRECT_COLLECTIONS no vale como universo. noShowPenalties lleva studentId dentro de la propia clave del documento, lo escribe un servicio real y aparece cero veces en ese inventario; construir el registro sobre esa lista habria hecho que la compensacion borrase un estudiante al que sus penalizaciones siguen apuntando. Son 20 las colecciones que firestore.rules nombra y el inventario no. El universo pasa a ser la union de ambas -unas 72- mas el fallo cerrado en tiempo de ejecucion que la especificacion ya pide, y cada clasificacion se apoya en el esquema del documento, no en que su modulo mencione studentId.",
    "PASAR A T099/T093: si esas 16 colecciones de inquilino no estan en el inventario de backup v3, si el backup las cubre no lo contesta T108. Aqui solo se midio: enrolmentRequests, enrolmentRequestHolds, noShowPenalties, shopOrders, shopProducts, lessonPlans, familyAchievements, familyGoals, disclaimers, disclaimerAcceptances, staffAvailability, staffAssignments, staffPermissionGrants, techniqueLibraries, regyfitMemberRecords y settings.",
    "El ejecutor de compensacion, el tercero de siete: escaneo de referencias acotado, las pruebas de borrado -ausencia previa, outputSetMac del recibo, contenido sin cambios, y la reserva solo con la linea base reabierta y su tupla probada ausente-, un bundle por transaccion, mas su runner, su adaptador y su ensayo en Emulator.",
    "Los otros cuatro ejecutores: rollback-projection -con su propio plan privado de cero escrituras-, rollback-readonly, canonical-recovery e identity-reconcile, este ultimo con sus dos protocolos, el acotado y el paginado.",
    "Acunar y consumir aprobaciones con lease, deadline y handoff de source; sin dual-write silencioso. Y cablearlas a cada adquisicion y a cada cierre.",
    "Reabrir y poner en verde los 18 requisitos RED heredados de T093 con dry-run y rollback en Emulator, y backup verificado antes de produccion.",
  ],
  T104: [
