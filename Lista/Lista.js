...
const RESOLUTION_REQUIREMENTS = {
  T127: [
    "Resuelta el 2026-09-07 la decision de producto: la firma presencial atestiguada se retira. El alta administrativa se apoya en la misma aceptacion digital que recoge la inscripcion de T121, asi que recordWitnessedWaiver y el metodo in_person_witnessed no se construiran. No hubo nada que borrar: solo existian en el analisis de T106.",
    "El alcance de la fila queda reducido a la renovacion digital desde /account/waiver.",
    "RESUELTO 2026-09-08: el operador eligio un parametro propio, BPT_WAIVER_REGISTRATION, con defineString y defecto disabled, para las siete callables de waiver y solo para ellas. El nombre importaba: BPT_SYNTHETIC_PILOT significa piloto sintetico, asi que llevarlo a produccion habria sido una afirmacion falsa sobre el entorno en vez de una decision sobre la capacidad. Salud, documentos privados y export agregado siguen con el gate sintetico.",
    "RESUELTO el 2026-09-08: R2 esta configurado de punta a punta. El operador aporto las dos credenciales, que quedaron en version 2 de R2_ACCESS_KEY_ID y R2_SECRET_ACCESS_KEY; se crearon ademas R2_ACCOUNT_ID, R2_BUCKET_NAME y R2_JURISDICTION, y los cinco se ligan a las cinco funciones que abren bucket. El bucket bptjersey existe con jurisdiccion EU, como exige la politica de residencia de T011, y r2-client.ts pasa a construir el endpoint jurisdiccional con fallo cerrado ante una jurisdiccion no reconocida. Verificado en produccion: la limpieza programada, que nunca habia funcionado, corre con HTTP 200 y cero 5xx. Pendiente derivado: rotar esa access key, porque su valor acabo en el transcript de la sesion via la notificacion de cambio de fichero del IDE, y con ella un redespliegue -las funciones fijan la version del secreto, no usan latest-.",
    "Con esas dos, implementar la renovacion digital segun docs/operations/t106-waiver-enrolment-integration-analysis.md, cuya via de firma presencial queda como analisis historico.",
  ],
  T126: [
  ...
  T058: [
    "CUMPLIDO 2026-09-08: verificacion en navegador con sesion real. Encontro que siete callables devolvian 401 por un permiso IAM ausente en la cuenta de servicio (firebaseappcheck.appCheckTokens.verify), no por la release. Concedido roles/firebaseappcheck.tokenVerifier y comprobado en el navegador del operador.",
    "PENDIENTE, no bloquea la fila pero queda escrito: dejar constancia del aviso a office y coaches. La release se hizo con esa casilla vacia, y el 8 del runbook dice que una casilla vacia es una release que no se hizo como dice el runbook.",
    "PENDIENTE: la quinta alerta del 6.1, fallos de Cloud Scheduler. Veinte intentos en cuarenta minutos y Google no publico el descriptor de la metrica pese a que el job ejecuto con exito; la sospecha razonable es que hace falta al menos un intento fallido para materializarla. Politica escrita y a una llamada.",
    "RESUELTO 2026-09-09: rotada la access key de R2. Los dos secretos en version 3 con un token nuevo del panel, cleanupExpiredMemberImportSessionsSchedule -la unica funcion desplegada que liga R2- redesplegada y atada a la 3, y las versiones 1 y 2 DESTROYED. El 200 de la limpieza NO probaba la credencial: solo llama a S3 si hay sesiones caducadas, y las cuatro colecciones estaban vacias, asi que se probo aparte con un ListObjectsV2 real contra el endpoint EU, con los valores por prompt oculto -sin disco, sin historial, sin transcript-. Solo entonces se destruyeron las viejas, porque Cloudflare muestra el secreto una sola vez y no habria vuelta atras.",
    "CERRADO 2026-09-09: el operador elimino el token viejo y confirmo el scope del nuevo en el panel. BPTJersey, applied to bptjersey en jurisdiccion EU, permiso Object Read & Write, Active, y ningun otro token en la lista. La credencial que usa produccion ve un solo bucket, el de la EU, con permisos de objeto y no de administracion: el minimo privilegio que pedia la especificacion.",
    "PENDIENTE, hallazgo del 2026-09-09: ningun despliegue no interactivo pasa del parametro BPT_WAIVER_REGISTRATION. Tiene default disabled, pero el CLI no lo asume sin TTY, no lo toma de una variable de entorno y exige un dotenv en el directorio fuente, que el predeploy borra entero. Afecta al runbook de release y a cualquier CI.",
    "PENDIENTE: aplicar Build watch paths en Pages (decision 9, autorizada y no aplicada). Wrangler no la expone, se hace en el panel: incluir apps/web/*, packages/*, pnpm-lock.yaml y package.json; excluir docs/*, tasks.md, Lista/*, graphify-out/*, qa/* y apps/functions/*.",
    ...
    "Lotes siguientes: 83 callables que la web invoca y no estan desplegadas, entre ellas el ciclo de facturacion manual completo (listFinancialAccount, getInvoice, issueManualInvoice, recordManualPayment, voidManualInvoice, savePaymentInstructions) y listMemberships. Cada lote con su autorizacion, su delta y sus precondiciones medidas POR FUNCION, no por lote: el anadido de ultima hora es el que nadie vuelve a mirar.",
  ],
  T059: [
    "CUMPLIDO 2026-09-08: T058 aprobada, con su fila de release completa y su verificacion en navegador. La evidencia existe y esta en tasks.md.",
    "CUMPLIDO 2026-09-08: LECCIONES.md existe en la raiz, con once lecciones y el caso que produjo cada una, y el analisis de brechas en docs/operations/t059-capability-gap-analysis.md, medido contra el proyecto real en vez de citado del ledger.",
  ...
  T108: [
    ...
    "CUMPLIDA 2026-09-08 la rebanada 9: la verificacion y el cierre del bootstrap enchufados a su runner, con adaptador Firestore y ensayo en Emulator, y con ellos las dos transiciones que le faltaban al padre, applying -> verified y verified -> completed. El artefacto congelado pone la pregunta y Firestore la respuesta, de modo que la comparacion final no prueba que el artefacto sea igual a si mismo. verify:mvp verde, Emulator 82/83, cinco mutaciones y cinco muertes.",
    "CUMPLIDA 2026-09-08 la rebanada 10: el ejecutor forward, el segundo de los siete y el primero que escribe documentos de dominio de verdad. Crea el estudiante migrado -active=false, status=inactive, sin familyId ni userId-, su perfil administrativo y las reservas de sus identificadores, los tres en la misma transaccion que el recibo, el plano de control y el padre. Todo lo derivable sale de la fila legada releida dentro del chunk; la fila revisada solo aporta lo que el origen no puede, y las ata sourceRowMac. verify:mvp verde, 276 ficheros y 2360 pruebas unitarias, Emulator 88/89, nueve mutaciones y nueve muertes.",
    "CORREGIDO 2026-09-08: forward NO pone filas en cuarentena. La especificacion rechaza el plan entero cuando una fila deja de ser elegible -cuadro de clasificaciones y paso 3 del algoritmo de confirmacion-, asi que quarantinedCount se queda en 0 igual que en bootstrap. La unica cuarentena del sistema es la del restore de backup v3, quarantined-no-auth, que es otra cosa.",
    "CORREGIDO 2026-09-08 el recuento: antes de esta rebanada iba UN ejecutor de siete, no dos. Con el forward van dos, y faltan cinco: compensacion, rollback-projection, rollback-readonly, canonical-recovery e identity-reconcile.",
    "CUMPLIDA 2026-09-08 la rebanada 11: los contratos del plan privado congelado y su prueba. El manifiesto revisado lista solo filas que se van a escribir, con la elegibilidad decidida por isWriteEligibleClassification y no por una segunda lista; priorAbsence es el literal true, de modo que un plan no puede ni expresar una sobrescritura; tres MAC con tres dominios -manifiesto, plan y conjunto de origen-, y el plan lleva dentro el MAC de su manifiesto, con lo que el par es autoligado. Los contadores se recomputan desde las filas. verify:mvp verde, 278 ficheros y 2384 pruebas unitarias, trece mutaciones y trece muertes.",
    "CORREGIDO 2026-09-08: la rebanada 11 NO trajo el almacen real. Trajo los contratos y la prueba, y ahora hay DOS puertos sin adaptador en vez de uno: MemberDirectoryBootstrapArtifactStore y MemberDirectoryFrozenPlanStore.",
    "RESUELTO 2026-09-08 por el operador: el artefacto congelado vive en un fichero bajo una raiz de entrada aprobada, alcanzable solo desde un Emulator demo en loopback, y toda ruta remota falla cerrada; si algun dia vive en R2 va con sobre cifrado propio encima del cifrado en reposo del bucket. Razon: el staging aislado se descarto el 2026-09-07 y la fase 3 no esta autorizada por T092, asi que un almacen remoto no tendria hoy ninguna ejecucion autorizada a la que servir.",
    "CUMPLIDA 2026-09-08 la rebanada 12: el almacen de artefactos, con los DOS puertos cerrados. El sobre AES-256-GCM sella antes de que los bytes lleguen a ningun sumidero -clase, academia y operacion son el additional data, y el texto en claro es el JSON canonico, los mismos bytes del MAC-; la escritura es create-only por wx y hay remove para borrar las fixtures al terminar el ensayo. La llave reutiliza MEMBER_DIRECTORY_BASELINE_ENCRYPTION_SECRET en vez de acunar una quinta. verify:mvp verde, 280 ficheros y 2404 pruebas unitarias, doce mutaciones y doce muertes.",
    "BORRADO 2026-09-08 un guarda que ninguna prueba podia matar: la derivacion de clave por clase de artefacto sobrevivio a su mutante, y no por falta de prueba sino porque la separacion por clase ya vive en el additional data, donde una apertura cruzada muere en el tag.",
    "CUMPLIDA 2026-09-09 la rebanada 13: el dry-run que produce el manifiesto. Dos pasadas: classifyMemberDirectoryForwardRows clasifica cada fila legada con cero escrituras y sin necesitar ninguna decision -es el informe del revisor, con las filas pendientes en awaitingDecision-, y planMemberDirectoryForwardDryRun repite la clasificacion con las decisiones dentro y emite manifiesto, plan y recibo, negandose si queda una fila elegible sin decidir. El plan se deriva invocando al propio ejecutor forward, asi que plan y ejecutor no pueden discrepar por construccion; y el dry-run prueba su propia salida con openMemberDirectoryFrozenPlan antes de devolverla. verify:mvp verde, 281 ficheros y 2440 pruebas unitarias, veintitres mutaciones y veintitres muertes.",
    "CORREGIDO 2026-09-09 un hueco real entre las rebanadas 10 y 11: el ejecutor forward exige trainingTimePreferences y se niega a inventarlas -el registro legado no tiene campo equivalente-, y el esquema del manifiesto no tenia ese campo, asi que sus filas se las fabricaba a mano una prueba. Anadido al manifiesto, obligatorio en un createable-adult y prohibido en una correspondencia, que no escribe documento de estudiante.",
    "AÑADIDA 2026-09-09 la prueba que le faltaba al MAC de hoja: que ligue la ruta ademas del contenido no lo discriminaba ninguna prueba del repositorio, y salio al sacar la hoja a su propia funcion para que la lean el recibo y el plan. Se le escribio la prueba en vez de borrar el guarda -al reves que en la rebanada 12- porque sin la ruta dos documentos identicos en rutas distintas pliegan a la misma hoja.",
    "CUMPLIDA 2026-09-09 la rebanada 14: el runner del forward. Abre los artefactos congelados, los prueba contra el recibo del padre, comprueba la linea base de identidad del paso 4, lee exactamente los documentos que el plan nombra y le pregunta al ejecutor que escribe. Dos actores a proposito: los documentos llevan el actor con el que se congelo el plan -sus MAC de contenido se tomaron sobre documentos con ese actor dentro- y la auditoria lleva a quien corre el chunk ahora. Las reservas esperadas salen de las rutas de destino del propio plan. verify:mvp verde, 282 ficheros y 2454 pruebas unitarias, once mutaciones y once muertes.",
    "ESCRITAS 2026-09-09 las dos pruebas que faltaban, en vez de borrar dos guardas: la comparacion de la raiz de salida contra la que el plan congelo caza el desfase de version -un plan congelado antes de un despliegue y ejecutado despues-, y el perfil administrativo ya existente en un destino, que es la ausencia previa rota entre la revision y la ejecucion.",
    "CUMPLIDA 2026-09-09 la rebanada 15: la verificacion y el cutover, pasos 10 y 11. Con esto el camino forward esta entero en codigo: el dry-run fabrica los artefactos, el runner corre los chunks, la verificacion los prueba y el cutover cambia el lector. El planificador del cutover no existia: rollbackEligibleStudentCount pasa a ser el postCutoverAdmittedStudentCount del recibo y no un conteo tomado en el momento, y globalLegacyReadEliminated se queda en false porque cambiar el lector y probar que nada depende ya de members son dos afirmaciones distintas y solo T097 puede hacer la segunda. La prueba se rederiva en las dos mitades: un padre marcado verified sobre recibos que ya no prueban nada no cutovea. verify:mvp verde, 283 ficheros y 2473 pruebas unitarias, veinte mutaciones y veinte muertes.",
    "CUMPLIDA 2026-09-09 la rebanada 16: los adaptadores Firestore y el ensayo de la operacion completa de punta a punta, que llevaba en la lista desde que se abrio la fila y ahora corre y pasa. El dry-run lee las filas legadas de Firestore y emite los artefactos, el almacen los sella a disco, el runner los abre a traves del cifrado y compromete un chunk, la verificacion lo prueba contra el plan y el cutover cambia el lector. Tres filas: dos altas y una correspondencia revisada contra un menor existente, que no se toca y solo gana su perfil. verify:mvp verde, test:integration 89/90 desde 88/89, cinco mutaciones y cinco muertes contra el Emulator.",
    "HALLAZGO 2026-09-09: el adaptador leia las relaciones de familyRelationships y la coleccion real es relationships. Ninguna prueba unitaria podia verlo -el nombre vive solo en el adaptador- y el primer ensayo tampoco, porque solo cubria altas y una alta no lee ninguna familia. Por eso el ensayo lleva una correspondencia con un menor.",
    "MEDIDO 2026-09-09: incluir integration/ en qa/tsconfig.json da 27 errores en 8 ficheros -waitlist-offer-transaction 9, firestore-adapters 6, membership-adapters 3, finance-adapters 3, waitlist-store 2, backup-rehearsal 2, plan-adapters 1, consent-waiver 1-, de tres causas: un scope que falta, firebase-functions/v2/https sin resolver, y un generico que ya no acepta argumento de tipo. Ninguno es de los seis ensayos del directorio de miembros. Arreglarlos toca el package.json de qa y no cabe en una rebanada de T108; el cambio se revirtio.",
    "SIGUIENTE, rebanada 17: el registro de cierre de referencias, member-directory-reference-closure-v1, que la compensacion necesita antes de existir. MEDIDO 2026-09-09 y es mas grande de lo que esta lista sugeria: tiene que decir de cada coleccion del inquilino -55 directas mas 3 anidadas, la lista autoritativa esta en TENANT_BACKUP_V3_DIRECT_COLLECTIONS- si puede referenciar a un estudiante y como se comprueba, o por que no puede, y hay ~25 modulos de dominio que llevan studentId contra los que auditarla. Es la unica pieza de T108 donde equivocarse borra datos en vez de fallar cerrado, asi que la lista de la especificacion no vale copiada. Despues, el ejecutor de compensacion.",
    "CERRADO 2026-09-09: el ensayo de la operacion completa de punta a punta ya no es un pendiente -corre y pasa contra un Firestore real, con dos altas y una correspondencia revisada contra un menor existente-. Van dos ejecutores de siete y dieciseis rebanadas, y el camino forward esta entero; lo que la fila debe es todo lo que deshace o repara una migracion.",
    "Escribir los cinco ejecutores que faltan del runbook member-directory-v1: compensacion, rollback-projection, rollback-readonly, canonical-recovery e identity-reconcile.",
...
const closeoutItems = [
  ...
  task(
    ...
    "T057",
    "La publicacion coordinada en produccion sigue pendiente. Estado real al 2026-09-07, leido en solo lectura: 39 funciones desplegadas en seis lotes con commit de origen reconstruido, Rules e indices identicos a HEAD, Pages en 51918ad, y los tres secretos del directorio canonico ya en version 2 con material real (el placeholder que esta fila registraba se resolvio ese dia). Hueco medido con pnpm release:delta: la web invoca 149 callables y 114 no estan desplegadas (no 25); dos desplegadas ya no existen en el codigo (searchMembers, getMemberReportSummary) y un --only functions sin nombres las borraria; ninguna funcion define BPT_SYNTHETIC_PILOT, asi que las tres de consentimiento desplegadas estan inertes. Procedimiento en docs/operations/t058-release-rollback-runbook.md. 2026-09-07: el operador tomo las cuatro decisiones abiertas segun lo recomendado -no llevar BPT_SYNTHETIC_PILOT, R2 fuera hasta tener credenciales, retirar las dos huerfanas y listRegyfitAccessRecords, y empezar por el lote sin gate de codigo- y se preparo y midio el primer lote: 31 callables por nombre (staff 9, families 3, crm 3, penalties 3, listPublicShopCatalog, announcements 9, listClientReminders, listRetentionAlerts, listUpcomingBirthdays). Dos exclusiones: createCrmLead, exportada pero que la web no invoca, y las cuatro de profiles/*, porque saveClientProfile resulto ser un escritor canonico de pleno derecho -la nota del runbook que decia lo contrario era falsa y quedo corregida- y desplegarla abriria una via por la que un cliente crea su propio estudiante sin pasar por la bandeja de solicitudes de T121. Precondiciones medidas: puertas locales verdes (verify:mvp completo, golden path 19/19, artefacto exit 0), delta medido, secretos en version 2 habilitada, ninguna de las 35 lee process.env, billingEnabled true, y compatibilidad de datos limpia porque los contratos de dominio son identicos entre los commits desplegados y HEAD. Falta lo que no pone el asistente: la autorizacion explicita para esta release, la ventana con aviso, y las cinco alertas del 6.1, cuyos cinco umbrales concretos quedaron escritos ese dia en el runbook a peticion del operador, que eligio el orden: primero las alertas, despues la autorizacion. 2026-09-08: la release ocurrio. El operador autorizo nombrando el lote y eligio ventana inmediata; se desplegaron las 31 por nombre en tres lotes (staff 9, core 10, comms 12) mas un cuarto con el redespliegue de la programada huerfana, los cuatro con Deploy complete y exit 0, sobre el commit 437e7a246c45ac61ef95458f157c91b6b85fc0c5 con arbol limpio. Antes se crearon cuatro de las cinco alertas del 6.1, que resultaron NO exigir roles ausentes: testIamPermissions concede alertPolicies, notificationChannels, uptimeCheckConfigs y billing.budgets a la identidad de desarrollo, asi que la nota del runbook que las declaraba del operador era falsa y quedo corregida; el presupuesto es de 200.000 COP mensuales y avisa sin cortar. Verificacion 4.4: functions:list 70 ACTIVE (39 previas mas las 31), las 31 presentes, las cuatro de profiles/* ausentes y las dos huerfanas intactas; sondeos anonimos 31 de 31 en 401, ninguno 404 ni 5xx; logs con STARTUP TCP probe succeeded y sin billing is disabled, Cannot find module ni error de secreto; delta posterior 83 invocadas sin desplegar y 97 exportadas sin desplegar, exactamente 31 menos en ambas. Dos hallazgos que la release destapo: no existia ningun job de Cloud Scheduler pese a figurar cleanupExpiredMemberImportSessionsSchedule como scheduled -era la etiqueta de despliegue, no un disparador-, asi que nunca se ejecutaba, y el redespliegue lo creo, ENABLED cada 15 minutos; y el uptime check del callable, creado con GET, hacia que el propio callable registrase un error cada 5 minutos desde dos regiones, asi que se recreo con POST y se repunto la politica antes de borrar el viejo. APROBADA el 2026-09-08 tras la verificacion en navegador del 4.4.4, hecha con Chromium visible contra produccion, el operador iniciando sesion a mano en /staff/login -no /login, que es la puerta de clientes- y la credencial sin pasar por el asistente. Esa verificacion encontro un fallo real que el sondeo anonimo no podia encontrar: siete callables devolvian 401 con sesion real. Se probaron tres hipotesis y las dos primeras eran falsas. (1) Los clientes web que no pedian token de App Check de un solo uso: se corrigio en no-show-penalties-client, staff-permissions-client y birthdays-client con sus pruebas y por mutacion, es correcto y NO era el bloqueo. (2) reCAPTCHA puntuando mal a un navegador automatizado: falso, fallaba igual en el navegador normal del operador. (3) La buena, y estaba en el log del servidor desde el principio: Permission firebaseappcheck.appCheckTokens.verify denied. La cuenta de servicio de las funciones tenia roles/editor y ningun permiso de App Check, asi que toda callable con consumeAppCheckToken llevaba rota EN PRODUCCION DESDE SIEMPRE -panel financiero, informes operativos, agenda, backup, export, perfil de tutor-, no solo las del lote. Se concedio roles/firebaseappcheck.tokenVerifier, que otorga ese unico permiso; tras la propagacion, cero rechazos y la cola de penalizaciones cargando en el navegador del operador. Verificacion completa: 70 ACTIVE con las 31 presentes y profiles/* ausentes, sondeos 31 de 31 en 401, logs limpios, delta de 114 a 83, y navegador con sesion real. LO QUE NO SE CUMPLIO Y QUEDA ESCRITO: el aviso previo a office y coaches del 4.0(10) no consta; el operador eligio ventana inmediata sobre una opcion que pedia confirmarlo. La fila se aprueba porque lo que mide -que el despliegue ocurriera, autorizado y verificado- ocurrio, y la casilla vacia queda en la fila de release. Release de frontend aparte: commit dd131ef, deployment de Pages 2dfc97be, anterior a53dfe90 para rollback.",
    [
      "tasks.md",
      "STACK.md",
      "docs/operations/t058-release-rollback-runbook.md",
      "docs/operations/t057-post-pilot-production-checklist.md",
      "docs/operations/release-437e7a2-functions.json",
  ...
  task(
    ...
    "T058",
    "APROBADA 2026-09-08 por evidencia: los dos entregables existen y se pueden abrir, LECCIONES.md en la raiz y docs/operations/t059-capability-gap-analysis.md. El analisis esta medido contra el proyecto real -release-delta.mjs contra bptjersey-f5a25- y no citado del ledger, y cada callable se asigno a su capacidad del BRIEF leyendo que modulo de apps/web/src/lib la invoca; las sumas cuadran con el delta: 70 desplegadas y 83 invocadas sin desplegar, exactas por capacidad. Su hallazgo es el del proyecto entero: el tablero mide 98 % de filas aprobadas y produccion sirve el 44 % de lo que la web invoca, 66 de 149; los dos numeros son correctos y miden cosas distintas, y el proyecto midio durante meses el primero. Produccion sirve hoy al 100 % las dos cosas que el BRIEF puso FUERA del MVP -retail 9/9, CRM y retencion 5/5- y al 0 % la capacidad 6 -niveles y promociones- y al 6 % la capacidad 3 -membresias y finanzas manuales-, ambas por delante de aquellas en el orden RICE del propio BRIEF; nadie decidio eso, salio de elegir cada lote por lo que estaba listo y era barato. Tres huecos que un recuento no ve: penalizaciones desplegadas sin la asistencia que las produce, panel financiero sin ciclo financiero detras, y alta administrativa sin waiver, descargos ni perfil de salud, que en un piloto con menores es el que mas pesa. Y una brecha que no es de codigo: el directorio canonico en produccion esta vacio de miembros reales, porque la migracion es T108. LECCIONES.md recoge once lecciones, cada una con el caso que la produjo, encabezadas por la que mas costo: un bloqueo que no se ha medido no es un bloqueo, es una hipotesis. Lo que la fila no afirma, escrito en el paragrafo 6 del analisis en vez de omitido: que las 70 desplegadas funcionen bien con sesion real, ni que las pantallas de las capacidades ausentes degraden todas como la unica observada, /admin/billing.",
    ["tasks.md"],
    "roadmap",
  ),
];
...
const recoveryItems = [
  ...
  task(
    ...
    "T093,T099",
    "Duodecima rebanada 2026-09-08: el almacen de artefactos, con los dos puertos cerrados. Decision del operador: el artefacto congelado vive en un fichero bajo una raiz de entrada aprobada, alcanzable solo desde un Emulator demo en loopback, y toda ruta remota falla cerrada, porque el staging aislado se descarto y la fase 3 no esta autorizada, de modo que un almacen remoto no tendria hoy ninguna ejecucion autorizada a la que servir. El sobre AES-256-GCM sella antes de que los bytes lleguen a ningun sumidero: clase de artefacto, academia y operacion son el additional data, asi que un manifiesto no se abre como plan ni bajo otra operacion ni para otro inquilino, y eso no es una comparacion que se pueda olvidar sino una entrada del tag; el texto en claro es el JSON canonico, los mismos bytes sobre los que se toma el MAC. La escritura es create-only por wx, porque un artefacto congelado reescribible en su sitio es la deriva que los MAC existen para cazar. La llave reutiliza MEMBER_DIRECTORY_BASELINE_ENCRYPTION_SECRET en vez de acunar una quinta, y el almacen comprueba al construirse que no coincide con las de identidad, integridad ni cursor. Se borro un guarda que ninguna prueba podia matar: la derivacion de clave por clase de artefacto, cuya separacion ya vive en el additional data. verify:mvp verde, 280 ficheros y 2404 pruebas unitarias desde 278 y 2384, doce mutaciones y doce muertes. Undecima rebanada 2026-09-08: los contratos del plan privado congelado y su prueba, que es lo que el ejecutor forward necesitaba y no tenia: hoy sus filas -destino, clasificacion, revision, preferencias y sourceRowMac- las fabricaba a mano una prueba. El manifiesto revisado lista solo filas que se van a escribir, porque la especificacion no da una tercera opcion al destino y la fase 1 solo confirma un plan integramente elegible; la elegibilidad la decide isWriteEligibleClassification y no una segunda lista. priorAbsence es el literal true, de modo que el plan no puede ni expresar una sobrescritura. Tres MAC con tres dominios -manifiesto, plan y conjunto de origen- porque responden a preguntas distintas, y el plan lleva dentro el MAC de su manifiesto, con lo que el par es autoligado. El sourceMac del recibo pasa a derivarse del manifiesto, asi que un origen cambiado se caza una vez para todo el plan y no chunk a chunk, y las raices de salida del recibo tienen que ser las del plan o el desajuste apareceria en el ultimo chunk con los anteriores ya escritos. Los contadores se recomputan: plannedNewStudentCount es exactamente las filas createable-adult. No trajo el almacen real: ahora hay dos puertos sin adaptador en vez de uno, y lo que falta para cerrarlos es una decision del operador sobre donde vive el artefacto encriptado y con que llave. verify:mvp verde, 278 ficheros y 2384 pruebas unitarias desde 276 y 2360, trece mutaciones y trece muertes. Decima rebanada 2026-09-08: el ejecutor forward, el segundo de los siete y el primero que escribe documentos de dominio de verdad. Crea el estudiante migrado -active=false, status=inactive, sin familyId ni userId, que son los invariantes 9 y 23-, su perfil administrativo y las reservas de sus identificadores, los tres en la misma transaccion que el recibo, el plano de control y el padre, por el envoltorio de la rebanada 5. La decision que le dio forma es de donde sale cada campo: todo lo derivable se toma de la fila legada releida dentro del chunk, de modo que el plan no puede colar un valor que el origen nunca tuvo, y la fila revisada aporta solo lo que el origen no puede -destino, clasificacion, revision, preferencias de horario y, para un menor, familia y relacion-; las ata sourceRowMac, con dominio propio y sobre el documento entero, asi que cualquier cambio del origen desde la revision es un unico fallo. Guarda el valor normalizado y no el crudo, porque la busqueda exacta recalcula el digest desde el valor del perfil y un crudo almacenado haria que toda reserva correcta contestara no-match. Se niega a un perfil que ya existe, a una correspondencia explicita cuyo destino es el identificador legado -la coincidencia de mismo ID sin su revision- y a una reserva que ya existe, aunque el dueno sea el mismo estudiante. Dos correcciones de partida: forward no pone nada en cuarentena -la especificacion rechaza el plan entero-, y el recuento de ejecutores iba en uno, no en dos. verify:mvp verde en una vuelta, 276 ficheros y 2360 pruebas unitarias desde 275 y 2340, Emulator 88/89 con la omitida de siempre, y nueve mutaciones con nueve muertes. Novena rebanada 2026-09-08: el cierre del bootstrap enchufado a su runner, con adaptador Firestore y ensayo en Emulator, y con el las dos transiciones que le faltaban al documento padre, applying -> verified y verified -> completed, cada una su propia transaccion. La decision que le dio forma es de donde salen las tuplas: si saliesen del artefacto congelado, la comparacion final probaria unicamente que el artefacto es igual a si mismo, asi que el artefacto pone la pregunta -que reservas planifico la operacion- y Firestore pone la respuesta, reconstruyendo cada tupla desde lo guardado. Exige ademas que el MAC del artefacto sea el que el recibo de dry-run del padre nombro: dos almacenes independientes diciendo lo mismo, o no hay cierre. La verificacion escribe solo al padre, bajo compare-and-set contra la revision y el MAC de evento sobre los que se establecio la prueba, y su statusAuditEventId nombra el evento en el que el plano de control esta parado, no uno nuevo, porque la cadena de guard tiene un evento por revision y mover el estado es lo unico que esta transaccion no puede hacer. El ensayo destapo que reintentar una operacion ya completada moria en la precondicion de cierre, que manda a buscar una averia del plano de control donde no hay ninguna: ahora el runner lee al padre antes de exigirla y contesta que ya esta completada. El formato de tupla y la precondicion de cierre pasan a tener una sola definicion. 275 ficheros y 2335 pruebas unitarias, Emulator 82/83, verify:mvp verde en una vuelta; por mutacion cinco reglas y cinco muertes, la quinta solo despues de escribirle la prueba que le faltaba. va rebanada 2026-09-07: el MAC de linea base de identidad, con dominio propio para que un MAC de salida de chunk y uno de linea base no verifiquen el uno como el otro, y que ordena las tuplas de modo que describe un conjunto y no una secuencia; la verificacion del bootstrap, que exige la secuencia de chunks sin huecos y recompone la linea base desde cero para compararla con el artefacto, y que es pura y se llama dos veces a proposito para que una caida entre verificar y cerrar reanude solo en el cierre y ese vuelva a establecer la prueba; y el cierre del bootstrap, que devuelve la tupla pre-cutover y no la canonica porque el bootstrap no migra nada, exige lease vivo y borra los campos de lease en vez de vaciarlos. 19 pruebas mas, 2312 unitarias, y un guarda inalcanzable borrado que encontro una prueba. Septima rebanada 2026-09-07: el documento de operacion padre, academies/{academyId}/memberDirectoryMigrations/{operationId}, que hasta entonces no existia: el chunk de bootstrap escribia claves y nada movia al padre. El recibo de dry-run va anidado entero, no desplegado en campos, asi que la inmutabilidad de identidad, versiones, MAC y contadores es estructural y no una convencion. frozen -> applying la mueve el primer chunk comprometido y solo el, en la misma transaccion y ligada al mismo evento de guard; un chunk posterior no devuelve el documento sin cambios sino nada; los de compensacion no mueven al padre; y uno cuya operacion esta verified, completed o failed falla cerrado. global-legacy-elimination queda fuera a proposito: no posee fase de chunk y su recibo liga otro conjunto de pruebas. 13 pruebas mas, 2294 unitarias, Emulator 80/80, y la regla frozen -> applying comprobada por mutacion. Sexta rebanada 2026-09-07: el primer ejecutor. El de bootstrap de claves de identidad, con sus escrituras de dominio enchufadas al envoltorio transaccional y su ensayo en Emulator, 5 pruebas contra Firestore real. Crea una reserva por cada identificador vigente en las cinco clases; preserva la clave compatible que ya existe en vez de reescribirla, que es lo que permite abandonar un bootstrap fallido sin borrar nada; y no pone nada en cuarentena: un registro ilegible, de otro inquilino, reasignado o inatribuible detiene el chunk y deja la congelacion en pie. El recibo del chunk pasa a probar lo que se escribio, porque el runner recomputa el MAC del conjunto de salida sobre las escrituras del ejecutor antes de leer nada. El adaptador Firestore convierte la decision del runner en un compare-and-set sobre revision y MAC de evento, que es lo que hace segura la division entre leer y comprometer. 273 ficheros y 2277 pruebas unitarias; Emulator 80/80. Corregido de paso un error de metodo: una asercion afirmaba probar el compare-and-set y fallaba antes por otra regla, asi que se aislo en una prueba propia y las tres reglas nuevas se comprobaron por mutacion. Falta la mayor parte: los otros seis ejecutores, el documento de operacion padre con sus transiciones, el plan privado congelado y su dry-run, la cuarentena y las aprobaciones. Arranque 2026-09-07, cinco rebanadas: contratos de migracion, ciclo de vida y lease, planificador de adquisicion, compromiso de chunk, y el envoltorio transaccional que valida la cadena estado-guard-evento y avanza estado, guard, recibo y auditoria juntos o nada. 70 pruebas focales; 272 ficheros y 2259 pruebas unitarias en verde. Queda: los siete ejecutores, cuarentena, aprobaciones, el adaptador Firestore y el ensayo en Emulator. Detalle: contratos de migracion, ciclo de vida y lease, planificador de adquisicion, y el compromiso de chunk con sus tres presupuestos (50 filas, 8 chunks, 400 filas) y la regla de replay: mismo recibo es no-op, recibo distinto falla cerrado. 63 pruebas focales, 614/614 del dominio. La criptografia y los vectores dorados ya existian de T093. Detalle: contratos de migracion, ciclo de vida y lease, y el planificador de adquisicion (tabla explicita de seis adquisiciones, dos de ellas ya congeladas; la proyeccion de rollback se niega tras el marcador global; el cambio forward->compensation conserva lease y resetea el contador de chunks). 56 pruebas focales, 607/607 del dominio. Detalle de las dos primeras: contratos de migracion y, encima, el ciclo de vida de la operacion (ocho estados como tabla cerrada, terminales, via inversa solo para directory-forward) y las reglas de lease (120 s, el vencimiento ya cuenta como expirado, nadie roba un lease vivo y uno expirado no lo renueva ni su dueno: exige recuperacion auditada). Corregidas dos cosas de la primera rebanada: la lista de fases con chunks y el recibo de chunk, que no ligaba status ni el chunk directo que la compensacion revierte. 39 pruebas focales, 590/590 del dominio. Entregados los contratos de migracion (nueve clasificaciones de dry-run con su elegibilidad, identificador y secuencia de chunk sin huecos, recibos de chunk y de dry-run sin PII por strictObject, y el limite de capacidad verificado en tiempo de plan), con 17 pruebas focales y 568/568 del dominio. Sigue pendiente porque falta la mayor parte: lease y transiciones de fase, escritor de chunks, los siete ejecutores, cuarentena, aprobaciones y el ensayo en Emulator; una rebanada entregada no es la tarea hecha. Alta 2026-09-05 al aprobar el corte de T093. Hereda los 11 requisitos out-of-cut (R06, R20-R22, R25, R26, R32-R34, R36, R45) y la clausula de migracion de los 7 partial (R07, R11, R12, R18, R23, R27, R31) de la matriz RED. Reconciliado 2026-09-07: sigue sin existir ningun ejecutor (ni escritor de memberDirectoryMigrationChunks ni acunacion o consumo de aprobaciones; solo el inicializador de baseline vacio y el ensayo de backup v3). El staging separado que esperaba se descarto con la enmienda de T057: el ensayo es el Emulator y la ejecucion real entra por la release de T058 con backup verificado y confirmacion separada. T099 en revision; T011 aprobada como borrador pero bloquea datos reales hasta la firma. No forma parte del piloto sintetico.",
    [
      "tasks.md",
      "docs/data/migrations/member-directory-v1.md",
      "docs/data/migrations/member-directory-v1-red-evidence.md",
