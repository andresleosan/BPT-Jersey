...
const RESOLUTION_REQUIREMENTS = {
  T127: [
    ...
    "Sacar consent-callables del gate BPT_SYNTHETIC_PILOT con un gate de produccion real. El 2026-09-07 esto se contesto en negativo para el primer lote de T058: el gate no se lleva a produccion, asi que consent-callables sigue desplegada e inerte.",
  ...
  T108: [
    ...
    "SIGUIENTE, rebanada 9: enchufar la verificacion y el cierre del bootstrap a su runner, con adaptador Firestore y ensayo en Emulator. Las piezas puras existen desde la rebanada 8 -el MAC de linea base, la verificacion y el cierre-; falta el cableado. Es el punto exacto donde quedo la sesion del 2026-09-08, que no toco T108.",
...
const closeoutItems = [
  ...
  task(
    ...
    "T058",
    "DESBLOQUEADA el 2026-09-08 al aprobarse T058: hasta ese dia su leccion era posterior a una produccion que no habia ocurrido, asi que no habia materia sobre la que escribir. Ahora la hay, y abundante. Sigue pendiente porque su evidencia es un LECCIONES.md que no existe: aprobarla afirmaria que existe un documento que se puede abrir. Material concreto que la sesion del 2026-09-08 deja para escribirlo: que una nota de runbook que declara un bloqueo sin medirlo cuesta releases enteras -las cinco alertas resultaron creables desde el primer dia-; que la precondicion 6 se mide por funcion desplegada y no por lote, porque el anadido de ultima hora es el que nadie vuelve a mirar; que un sintoma no nombra su causa, con tres casos del mismo dia -scheduled era una etiqueta sin disparador, journal unavailable era un indice ausente, y 401 era un permiso IAM del servidor-; que leer el log del que falla va antes que razonar sobre el que llama, porque dos hipotesis cayeron antes de mirar un log que daba la respuesta en texto plano; que toBeDefined no es una prueba de configuracion y que un doble que descarta sus argumentos no puede ver lo que le pasan; que un monitor mal construido genera el error que vigila; y que las alertas se pagaron solas, cazando una funcion rota desde agosto una hora despues de existir.",
    ["tasks.md"],
    "roadmap",
  ),
];
