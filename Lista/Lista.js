...
const IMPLEMENTATION_OVERRIDES = {
  ...
  T010: {
    ...
    implementationEvidence:
      "Paquete sintï¿½tico de proveedor creado para Emulator/isolated staging: tres candidatos ficticios, lï¿½mites de checkout/webhooks, costos y alertas; no prueba Jersey, no crea cuenta y T010 sigue bloqueada hasta cotizaciï¿½n y aprobaciï¿½n real.",
  },
  T008: {
    implementationStatus: "parcial",
    implementationEvidence:
  ...
  T055: {
    implementationStatus: "parcial",
    implementationEvidence:
      "verify:mvp local pasa: build normal y build:e2e-synthetic, unitarias 154/1066, Rules 8/64, carga sintetica 240 solicitudes/concurrencia 24 sin fallos (p95 41 ms, p99 47 ms, max 49 ms), E2E smoke 5 pasan/1 omitido; integracion waiver 1/1 y E2E waiver 4/4; T055 sigue bloqueada por T008/T009/T011 y carga live/staging.",
  },
  T082: {
    implementationStatus: "verificada",
    implementationEvidence:
...
const phase0Items = [
  ...
  task(
    ...
    "bloqueada",
    "Elegir el proveedor manteniendo los pagos detrÃ¡s de un adaptador independiente del proveedor.",
    "-",
    "Paquete sintï¿½tico de proveedor creado; T010 sigue bloqueada hasta cotizaciï¿½n, disponibilidad en Jersey, tï¿½rminos y aprobaciï¿½n real.",
...
const membershipItems = [
  ...
  task(
    ...
    "T010,T012",
    "Contrato provider-independent y adapter sintetico implementados; proveedor unconfigured fail-closed, payload sin tarjeta/PII, idempotencia tenant-scoped, pruebas focalizadas 6/6; verify:mvp 159/159 archivos, 1082/1082 tests, Rules 64/64, carga p95 82 ms y E2E 5 passed/1 skipped; T010 sigue bloqueada y no hay proveedor, credenciales ni cobro real.",
...
const crmItems = [
  ...
  task(
    ...
    "T019,T043",
    "Persistencia de timeline, idempotencia por eventKey, parser y callable implementados; duplicado conflictivo falla cerrado; pruebas focalizadas 10/10, typecheck Functions/Web, regresion y verify:mvp pasan; UI y datos reales no activados; no bloquea T056.",
    ["tasks.md", "BRIEF.md", "docs/operations/t043-t044-crm-synthetic-scope.md"],
    "roadmap",
  ),
  task(
  ...
  task(
    ...
    "T045",
    "Frontera provider-independent, historial tenant-scoped y fallback unconfigured verificados; unit 937/937, deploy-runtime 2/2, typecheck/lint/build/audit OK.",
    ["tasks.md", "STACK.md"],
    "roadmap",
  ),
  task(
...
const closeoutItems = [
  ...
  task(
    ...
    "Gates locales completos: verify:mvp, unitarias 159/1082, Rules 64/64, carga sintetica 240 solicitudes/concurrencia 24 sin fallos (p95 82 ms) y E2E smoke 5 pasan/1 omitida; T055 en revision para piloto sintetico; T011/live/staging/produccion siguen bloqueados.",
    ["tasks.md", "STACK.md"],
    "mvp",
  ),
  task(
    ...
    "T055",
    "Piloto E2E sintetico ejecutado: 71 pasaron, 14 omitidos por live/staging u opt-in y 0 fallos; verify:mvp y carga sintetica pasan; borrador de acta en docs/operations/t056-pilot-operator-acta-draft.md; falta firma del operador.",
    [
      "tasks.md",
      "BRIEF.md",
  ...
  task(
    ...
    "T056",
    "Checklist actualizado con verify:mvp 159/1082, Rules 64/64, carga sintetica p95 82 ms, runtime 2/2, seguridad y rollback local; firma T056, T011, staging real, costos/alertas y autorizacion productiva siguen pendientes.",
...
const roadmapV2Items = [
  task(
    "T060",
    "Booking avanzado, listas de espera, crÃ©ditos y reservas recurrentes",
    "pendiente",
    "Ampliar reservas despuÃ©s del MVP; el corte bÃ¡sico de una hora pertenece a T027.",
    "-",
    "Elemento de ruta futura; no implica aprobaciÃ³n del MVP.",
    ["tasks.md", "BRIEF.md"],
    "roadmap",
  ),
  task(
