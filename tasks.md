...
| T107 | Purgar el dataset real de miembros del historial git, del remoto y del arbol de trabajo                                 | T103,T104                                              | revision    | 2026-09-04: archivo local borrado; historial de main reescrito con git filter-repo y publicado con force-push con lease (origin/main = 2ea48ea + ab2e20e, 88 commits, 0 referencias al blob); deployment de Pages 95d8b4d3 borrado; objetos antiguos purgados en el repo local y bundle de respaldo eliminado. Pendientes del operador: solicitar a GitHub la purga de objetos colgantes y valorar la notificacion conforme a T011.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
### Pendiente al retomar (2026-09-05, tras cerrar T111 a T115)

Sustituye a los checklists anteriores del dia. Arbol limpio tras `50cc3bc` (T109), `7821ecb` (T110),
`d7f9b56` (T111 backend/web), `5e8d13e` (E2E de T111), `2961e8e` (T112), `4a3b997` (T113),
`5158bba` (T114) y el commit de T115. `pnpm verify:mvp` verde (253 archivos / 1912 unitarias,
91 Rules, carga 240/240 con p95 27 ms, smoke 5+1) y golden path 15/15 en Emulator con las siete
suites.

Decisiones que solo puede cerrar el operador:

- [ ] Revisar y aprobar o devolver T097, T098, T109, T110, T111, T112, T113, T114 y T115, las nueve
      en `revision` con evidencia en Emulator.
- [ ] Aprobar T104 y T107, que siguen en `revision`.
- [ ] T109: grabar las coordenadas reales de Town y West en `/admin/classes`; el sistema no inventa
      ninguna y hasta entonces todo check-in queda con senal `unavailable`.
- [ ] T110: habilitar el barrido de quorum automaticamente (funcion programada) es un checkpoint
      aparte; hoy existen el callable de staff y el runner manual de ventana, ambos idempotentes.
- [ ] T111: proponer las penalizaciones de una sesion tambien es manual (callable de staff). Si el
      operador quiere que se propongan solas al cerrar la sesion, es el mismo checkpoint de
      automatizacion que T110.
- [ ] T112: el panel no muestra la edad que cumple nadie, por decision de alcance minimo. Si el
      operador la quiere en el tatami, es una linea en la proyeccion; se deja anotado, no hecho.
- [ ] T113: la franja de edad filtra propuestas, pero `openStudentLevel` sigue permitiendo abrir un
      nivel fuera de franja y `approvePromotion` no consulta la regla. Es lo que pedia la fila; si
      el operador quiere que la franja tambien bloquee la apertura o la aprobacion, es una decision
      suya y un cambio pequeno en los dos callables.
- [ ] T114: la ventana de ocho semanas, las dos asistencias que definen habito y la tolerancia de
      media hora son valores elegidos, no dictados por el DOCX. Si el operador los quiere distintos,
      son tres constantes del dominio.
- [ ] T115: el coach deja de ver la nota clinica. Si alguna operacion real dependia de que la
      viera, es una decision de producto del operador y no un fallo tecnico.
- [ ] T010 y T011 siguen bloqueando T035/T036/T061, T099, T058 y el paso 2 de T106; sin T099 no hay
      recorrido de navegador ni datos reales. T117 depende ademas del texto legal.
...
- [ ] T116: permisos administrativos delegados a un coach por concesion auditada y revocable, sin
      ampliar el claim y conservando el fail-closed por rol. Es la unica de las lagunas que queda
      sin bloqueo externo.
- [ ] T117 esta parcialmente bloqueada: el mecanismo de disclaimers y su aceptacion por participante
      se puede construir, pero el texto legal depende de T011, que sigue sin decision.
- [ ] T097 restante: comparacion opt-in de adultos (sin modelo de opt-in) y reinicio de la barra de
      progresion al promover (DOCX, no verificado).
