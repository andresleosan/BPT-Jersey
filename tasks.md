...
| T091 | Corregir vistas administrativas para mostrar solo datos conectados | T021,T024,T030,T047 | revision | Overview, Activities, Groups, Attendance y CRM eliminan fallbacks sintéticos y muestran estados explícitos de carga, error o vacío. Falta desplegar/verificar los callables actuales y revisar CORS/IAM en producción. |
...
- TDD: RED reprodujo la ausencia del módulo de opciones; GREEN pasó 1/1. Las pruebas focalizadas de Auth/RBAC y endpoints afectados pasan 65/65; la suite web/node pasa 1305/1305 al excluir únicamente qa/unit/lista-progress.test.ts, que espera un contador obsoleto de Lista.
...
- Diagnóstico remoto previo: el bundle de Pages todavía era anterior y los callables nuevos respondían 404; listRegyfitAccessRecords fallaba el preflight CORS/IAM. Falta publicar el commit y desplegar las funciones corregidas para cerrar la verificación remota.
- T091 queda en revision hasta confirmar el despliegue, el preflight CORS y la carga autenticada del panel en producción. No se registraron credenciales, PII ni datos del scraper.
- Plan de release y rollback: publicar solo los callable listados en el comando de Firebase, sin migraciones ni escrituras de datos; comprobar preflight y petición sin Auth. Si falla, redeployar esas mismas funciones desde el commit previo conocido `0cd2446` y revertir el frontend mediante el mecanismo de Pages, sin tocar Firestore.