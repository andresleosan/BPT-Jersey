...
if (
  ...
  throw new Error("T066 runner requires explicit local demo-project emulator flags.");
}
...
if (
  ...
  throw new Error("T066 runner ports must match the active loopback emulators.");
}

required("AUTH_EMULATOR_E2E_EMAIL");
const password = required("AUTH_EMULATOR_E2E_PASSWORD");
if (password.length < 12)
  throw new Error("T066 runner requires a synthetic password of 12+ characters.");

const suiteEnvironment = {
  ...process.env,
  AUTH_EMULATOR_E2E: "true",
  AUTH_EMULATOR_E2E_ROLE: "headCoach",
...
run(["qa/scripts/seed-lesson-planning-emulator.mjs"]);
run([
  "qa/run-e2e.mjs",
  "tests/lesson-planning-auth-emulator.spec.ts",
