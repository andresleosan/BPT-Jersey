...
// T094 authenticated Emulator E2E for the client onboarding path: adult profile, guardian profile,
// family with a minor, waiver acceptance with PDF evidence, evidence download and revocation.
// Run inside `firebase emulators:exec --only auth,firestore,functions` with the flags below.
const repositoryRoot = resolve(import.meta.dirname, "../..");
const projectId = "demo-bpt-jersey";

if (
  process.env.T094_ONBOARDING_EMULATOR_E2E !== "true" ||
  process.env.GCLOUD_PROJECT !== projectId ||
  process.env.NEXT_PUBLIC_ADMIN_E2E
) {
  throw new Error("T094 runner requires explicit local demo-project emulator flags.");
}
// The waiver callables are closed outside the synthetic pilot. The Functions Emulator inherits the
// shell environment of `emulators:exec`, so the flag has to be present here as well.
if (process.env.BPT_SYNTHETIC_PILOT !== "true") {
  throw new Error("T094 runner requires BPT_SYNTHETIC_PILOT=true in the emulator environment.");
}
...
const functionsPort = process.env.T094_FUNCTIONS_EMULATOR_PORT?.trim() ?? "5001";
if (!/^[1-9]\d{3,4}$/u.test(functionsPort)) {
  throw new Error("T094_FUNCTIONS_EMULATOR_PORT must be a non-privileged local port.");
}

const academyId = required("T094_E2E_ACADEMY_ID");
const ownerEmail = required("T094_OWNER_EMAIL");
required("T094_ADULT_EMAIL");
required("T094_GUARDIAN_EMAIL");
const password = required("T094_E2E_PASSWORD");
if (password.length < 12) {
  throw new Error("T094 runner requires a synthetic password of 12+ characters.");
}
for (const name of [
...
// Clients: Auth users and claims only; their documents are created by the onboarding callables.
run(["qa/scripts/seed-onboarding-emulator.mjs"]);
// The canonical directory must exist before any profile or family writer can commit.
run([
