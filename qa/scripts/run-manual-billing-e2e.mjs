...
// T095 authenticated Emulator E2E for the manual billing cycle on canonical data: plan, adult
// membership, manual invoice, manual payment and account. Run inside
// `firebase emulators:exec --only auth,firestore,functions` with the flags below.
const repositoryRoot = resolve(import.meta.dirname, "../..");
const projectId = "demo-bpt-jersey";

if (
  process.env.T095_MANUAL_BILLING_EMULATOR_E2E !== "true" ||
  process.env.GCLOUD_PROJECT !== projectId ||
  process.env.NEXT_PUBLIC_ADMIN_E2E
) {
  throw new Error("T095 runner requires explicit local demo-project emulator flags.");
}
...
if (process.env.BPT_SYNTHETIC_PILOT !== "true") {
  throw new Error("T095 runner requires BPT_SYNTHETIC_PILOT=true in the emulator environment.");
}
...
const functionsPort = process.env.T095_FUNCTIONS_EMULATOR_PORT?.trim() ?? "5001";
if (!/^[1-9]\d{3,4}$/u.test(functionsPort)) {
  throw new Error("T095_FUNCTIONS_EMULATOR_PORT must be a non-privileged local port.");
}

const academyId = required("T095_E2E_ACADEMY_ID");
const ownerEmail = required("T095_OWNER_EMAIL");
const adultEmail = required("T095_ADULT_EMAIL");
const password = required("T095_E2E_PASSWORD");
if (password.length < 12) {
  throw new Error("T095 runner requires a synthetic password of 12+ characters.");
}
for (const email of [ownerEmail, adultEmail]) {
  if (!email.endsWith("@example.test")) throw new Error("T095 runner requires synthetic users.");
}
for (const name of [
...
// consent, membership and invoices are created by the callables under test.
run(["qa/scripts/seed-onboarding-emulator.mjs"], {
  ...
  T094_GUARDIAN_EMAIL: `t095-unused-guardian@example.test`,
});
run([
