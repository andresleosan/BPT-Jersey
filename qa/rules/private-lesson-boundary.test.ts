...
const projectId = `demo-bpt-jersey-private-lesson-boundary-${process.pid}`;
const academyIds = Object.freeze(["academy-a", "academy-b"] as const);
const privateLessonCollections = Object.freeze([
  "privateLessonPurchases",
  "privateLessonCreditUses",
] as const);
  ...
  await testEnvironment.withSecurityRulesDisabled(async (context) => {
    ...
    await Promise.all(
      academyIds.flatMap((academyId) =>
        privateLessonCollections.map((collectionName) =>
...
async function assertPrivateLessonCollectionDenied(
  firestore: RulesFirestore,
  academyId: string,
  collectionName: (typeof privateLessonCollections)[number],
): Promise<void> {
...
describe("private lessons direct Firestore boundary", () => {
  for (const actor of actorCases) {
    it(`denies every private lesson path operation for '${actor.name}'`, async () => {
      const context =
        actor.uid === null
      ...
      for (const collectionName of privateLessonCollections) {
        await assertPrivateLessonCollectionDenied(firestore, "academy-a", collectionName);
      }
    });
  }
});
