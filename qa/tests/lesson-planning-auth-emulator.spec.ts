...
test.describe("T066 lesson planning with Firebase Emulators", () => {
  test("authenticates a head coach, reviews and approves a persisted plan @critical", async ({
    page,
  }) => {
    test.skip(
      process.env.T066_LESSON_PLAN_UI_EMULATOR_E2E !== "true" ||
        !process.env.AUTH_EMULATOR_E2E_EMAIL ||
        !process.env.AUTH_EMULATOR_E2E_PASSWORD,
      "Synthetic T066 Emulator credentials and seed are required.",
    );
    ...
    await page.getByRole("button", { name: "Load lesson plan" }).click();
    await expect(
      page.getByRole("heading", { name: "Synthetic guard passing review" }),
    ).toBeVisible();
    await expect(page.getByText("Guard pass", { exact: true })).toBeVisible();
