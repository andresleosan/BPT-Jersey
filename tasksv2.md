...
| ID   | Tarea atomica | Depende de | Estado | Evidencia de salida |
| ---- | ------------- | ---------- | ------ | ------------------- |
| T012V2 | Reparar la cola de aprobacion de nuevos miembros | - | pendiente | **La fila mas urgente del ledger: hoy no se puede dar de alta a nadie.** La foto del operador muestra "Unable to open this request." y los tres botones -"Read the full request", "Send back to applicant", "Approve and enrol"- inertes en `apps/web/src/app/admin/members/requests/page.tsx`. Los callables existen y estan cableados en `apps/web/src/lib/enrolment-client.ts` (`getEnrolmentRequestDetail`, `approveEnrolmentRequest`, `returnEnrolmentRequest`), asi que el fallo esta en la ejecucion, no en la ausencia de codigo. La fila **empieza por diagnosticar en produccion** el error real que devuelve la callable -parametro de entorno cerrado, App Check, claims del administrador o permisos de Firestore- antes de tocar la interfaz. No se cierra con un mensaje de error mas bonito: se cierra aprobando a un solicitante de verdad. |
