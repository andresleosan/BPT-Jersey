...
import {
  memberAgeOn,
  needsAdultClaim,
  teenAccountMinimumAge,
  type MemberAccessService,
} from "@bpt-jersey/domain/members/access";
import {
...
const visibilityInputSchema = z.strictObject({
  studentId: z.string().min(1).max(128),
  showToMembers: z.boolean(),
});

const settingsPath = (academyId: string, studentId: string) =>
  `academies/${academyId}/memberPublicSettings/${studentId}`;
const teenAccessPath = (academyId: string, studentId: string) =>
...
export type StudentAccountLinker = (
  input: Readonly<{
    actor: MemberActor;
    studentId: string;
    userId: string | null;
...
export async function readPublicSettings(
  ...
  academyId: string,
  studentId: string,
): Promise<MemberPublicSettingsDoc> {
...
export type AccountSettingsDependencies = Readonly<{
  ...
  access: Pick<MemberAccessService, "authorise"> &
    Partial<Pick<MemberAccessService, "listProfiles">>;
...
export function createAccountSettingsService(deps: AccountSettingsDependencies) {
  ...
  ): Promise<{ studentId: string; permission: Permission; via: "self" | "guardian" }> {
  ...
  function readTeenAccess(
    ...
    return {
      ...
      adultClaimedAt: nullableString(data.adultClaimedAt),
  ...
  async function ownTeenAccess(actor: MemberActor) {
    if (!deps.access.listProfiles)
      throw new HttpsError("unavailable", "Your account could not be checked. Try again.");
    const own = (await deps.access.listProfiles(actor.academyId, actor.userId)).find(
      (profile) => profile.via === "self",
    );
    if (!own) return null;
    ...
    const required = needsAdultClaim({
      age,
      via: "self",
      createdByGuardian: doc !== null && doc.uid === actor.userId,
      adultClaimedAt: doc?.adultClaimedAt ?? null,
    });
  ...
  async function requireManage(actor: MemberActor, requestedStudentId: string): Promise<string> {
    ...
    if (permission !== "manage")
      throw new HttpsError("permission-denied", "Ask your guardian to change this.");
    return studentId;
  }
  ...
  return {
    async uploadProfilePhoto(
      actor: MemberActor,
      data: unknown,
    ): Promise<{ photoUrl: string | null; pending: boolean }> {
      ...
      try {
        ...
        if (error instanceof PhotoRejected)
          throw new HttpsError("invalid-argument", photoRejectedMessage);
        throw error;
      }
      ...
      try {
        previous = await db.runTransaction(async (tx) => {
          ...
          const next: MemberPublicSettingsDoc =
            permission === "manage"
              ? {
                  ...current,
                  photoObjectKey: objectKey,
                  photoConsentAt: at,
                  photoConsentBy: actor.userId,
                  updatedAt: at,
                }
    ...
    async approveProposedPhoto(
      ...
      const { approved, previous } = await db.runTransaction(async (tx) => {
        ...
        if (!current.pendingPhotoObjectKey)
          throw new HttpsError("failed-precondition", "There is no proposed photo to approve.");
        tx.set(ref, {
          ...current,
          photoObjectKey: current.pendingPhotoObjectKey,
          photoConsentAt: at,
          photoConsentBy: actor.userId,
          pendingPhotoObjectKey: null,
          updatedAt: at,
    ...
    async getMySettings(actor: MemberActor, data: unknown) {
      const { studentId, permission } = await permissionFor(
        actor,
        parse(studentInputSchema, data).studentId,
      );
      ...
      return {
        ...
        teenAccess: email
          ? { email, active: teen.data?.revokedAt === null || teen.data?.revokedAt === undefined }
          : null,
      };
    },
    ...
    async createTeenAccess(actor: MemberActor, data: unknown): Promise<{ email: string }> {
      ...
      if (via !== "guardian")
        throw new HttpsError("permission-denied", "Only a guardian can set up own access.");
      const { student, age } = await studentAge(actor.academyId, studentId);
      if (age === null || age < teenAccountMinimumAge || age >= 18)
        throw new HttpsError("failed-precondition", teenAgeMessage);
      ...
      const phoneNumber =
        student.phoneNumber ?? (guardian.ok ? guardian.value.phoneNumber : undefined);
      if (!phoneNumber)
        throw new HttpsError("failed-precondition", "Add a phone number to your profile first.");
      ...
      const at = now();
      try {
        await teen.auth.setCustomUserClaims(uid, {
        ...
        try {
          ...
          warn("Teen access rollback could not unlink the student", {
            academyId: actor.academyId,
            studentId,
          });
        }
        await db
          .doc(userPath(actor.academyId, uid))
          .delete()
          .catch(() => undefined);
        await teen.auth.deleteUser(uid).catch(() =>
          warn("Teen access rollback could not delete the Auth user", {
            academyId: actor.academyId,
            studentId,
            uid,
          }),
    ...
    async revokeTeenAccess(actor: MemberActor, data: unknown): Promise<Record<string, never>> {
      const teen = teenWriterDeps();
      const { studentId, via } = await permissionFor(
        actor,
        parse(studentInputSchema, data).studentId,
      );
      if (via !== "guardian")
        throw new HttpsError("permission-denied", "Only a guardian can remove own access.");
      ...
      if (!doc || doc.revokedAt !== null)
        throw new HttpsError("not-found", "This member has no own access to remove.");
      if (doc.adultClaimedAt !== null)
        throw new HttpsError("failed-precondition", "This account now belongs to the member.");
      const at = now();
      try {
        await teen.auth.updateUser(doc.uid, { disabled: true });
        await teen.auth.revokeRefreshTokens(doc.uid);
        await db
          .doc(userPath(actor.academyId, doc.uid))
          .update({ active: false, updatedAt: at, updatedBy: actor.userId });
        const { student } = await studentAge(actor.academyId, studentId);
        if (student.userId === doc.uid)
          await teen.linkStudentAccount({
            actor,
            studentId,
            userId: null,
            expectedUserId: doc.uid,
            now: at,
          });
    ...
    async getAdultClaimStatus(
      actor: MemberActor,
      data: unknown,
    ): Promise<{ required: boolean; studentId: string | null }> {
      parse(z.strictObject({}), data ?? {});
      const own = await ownTeenAccess(actor);
      return own?.required
        ? { required: true, studentId: own.studentId }
        : { required: false, studentId: null };
    },
    ...
    async claimAdultAccount(
      ...
      const own = await ownTeenAccess(actor);
      if (!own || !own.required || own.studentId !== input.studentId)
        throw new HttpsError("failed-precondition", "There is nothing to claim on this account.");
      const at = now();
      ...
      await teen.auth.setCustomUserClaims(actor.userId, {
        academyId: actor.academyId,
        role: "adultStudent",
      });
