...
export async function getCourseCalendarPage(db: Firestore, actor: CourseActor, input: {studentId: string; from: string; to: string; cursor?: string | undefined}) {
  ...
  if (cursor?.s) {const snapshot = await enrolments.doc(cursor.e).get(); if (snapshot.exists) enrolment = snapshot.data() as CourseEnrolment;}
    ...
    let query = enrolments.where("studentId", "==", input.studentId).where("status", "in", ["approved", "withdrawal_requested"]).orderBy(FieldPath.documentId());
  ...
  if (enrolment.studentId !== input.studentId || !["approved", "withdrawal_requested"].includes(enrolment.status)) courseFailure("forbidden", "This course enrolment is unavailable.");
  const course = courseData<Course>(await courseCollection(db, actor.academyId, "courses").doc(enrolment.courseId).get());
  if (!["published", "completed"].includes(course.status)) return {...empty, cursor:nextEnrolment};
  ...
  const sessions = scanned.map(s => s.data() as SessionRecord).filter(s => s.coursePublicationRevision === course.publicationRevision && canAccessCourseSession(enrolment!, s));
  ...
  for (const session of sessions) {
    ...
    if (booking && (booking.schemaVersion !== "2" || booking.source.enrolmentId !== enrolment.enrolmentId)) courseFailure("conflict","A course booking needs office review.");
    const at=enrolment.approvedAt!;
    bookings.push(booking?.status === "confirmed" ? booking : {bookingId:buildBookingId(session.sessionId,input.studentId),academyId:actor.academyId,sessionId:session.sessionId,studentId:input.studentId,membershipId:null,schemaVersion:"2",source:{kind:"course",courseId:course.courseId,enrolmentId:enrolment.enrolmentId},absent:booking?.absent??false,status:"confirmed",requestedAt:at,createdAt:at,createdBy:"course-access",updatedAt:at,updatedBy:"course-access",cancelledAt:null,cancellationReason:null});
