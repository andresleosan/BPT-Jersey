...
export function IntroApplicationForm() {
  const [context, setContext] = useState<IntroMembershipContext>();
  const [site, setSite] = useState<Site>("Town");
  ...
  const existing = conversion
    ? context?.applications.find((item) => item.conversionId === conversion.conversionId)
    : context?.applications[0];
  const plans = useMemo(
    () => context?.plans.filter((plan) => plan.classSites.includes(site)) ?? [],
    [context, site],
  );
  useEffect(() => {
    setPlanId((current) =>
  ...
  if ((existing && existing.status !== "needs_correction") || status === "done")
    return (
      <section className="client-trial-form">
        <h2>Membership pending office approval.</h2>
        <p>Your receipt is private. The office will review the selected plan and payment.</p>
      </section>
    );
  if (!conversion || !context?.instructions)
    return (
      <section className="client-trial-form">
        <h2>Membership application unavailable</h2>
        <p>Complete an Intro Class first or ask the office to review your account.</p>
      </section>
    );
  return (
    <form className="client-trial-form" onSubmit={(event) => void submit(event)}>
      <h2>
        {existing?.status === "needs_correction"
          ? "Update your membership application"
          : "Choose your membership"}
      </h2>
      {existing?.status === "needs_correction" ? (
        <p role="status">
          {existing.decisionReason ?? "The office asked you to update the application."}
        </p>
      ) : null}
      <label htmlFor="intro-site">Training centre</label>
      <select
        id="intro-site"
        value={site}
        onChange={(event) => setSite(event.target.value as Site)}
      >
        <option value="Town">Town</option>
        <option value="West">West</option>
      </select>
      <label htmlFor="intro-plan">Membership plan</label>
      <select
        id="intro-plan"
        value={planId}
      ...
      >
        {plans.map((plan) => (
          <option key={plan.planId} value={plan.planId}>
            {plan.displayName} — {formatPlanPrice(plan)}
          </option>
        ))}
      </select>
      ...
      <button
        ...
        type="submit"
      >
        {status === "busy" ? "Sending..." : "Send for approval"}
      </button>
      {status === "error" ? (
        <p role="alert">The application could not be sent. Check the details and try again.</p>
      ) : null}
    </form>
  );
}
