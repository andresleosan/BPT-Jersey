...
/**
 * D12 / Q5 (2026-09-24): loads the terms status for every participant on the account once. It no
 * longer blocks the whole account; each participant is blocked on their own calendar through
 * `DisclaimerBlock`, and Settings, My plan and logout stay reachable (other routes, R14).
 */
export function WaiverGate({
  children,
}: Readonly<{
  children: (gate: (studentId: string) => React.ReactNode | null) => React.ReactNode;
}>) {
  const [status, setStatus] = useState<DisclaimerStatus | "error">();
  const [attempt, setAttempt] = useState(0);

  useEffect(() => {
    let active = true;
    void getMyDisclaimerStatus()
      .then((value) => {
  ...
  // DisclaimerBlock: only a loaded status naming this participant as clear lets the calendar show.
...
function pending(participant: DisclaimerStatus["participants"][number]): boolean {
  return !participant.terms || !participant.disclaimers;
}

/**
 * Stands in for the calendar of one participant. Fails closed: while loading a skeleton, and when
 * the check failed (or does not know this participant) a Retry card, never the calendar.
 */
  ...
  if (session?.role === "teenStudent") {
    return (
      <div className="calendar-error terms-block" role="status">
        <p>Your parent or guardian needs to accept the academy terms first.</p>
      </div>
    );
  }
  return (
    <div className="calendar-error terms-block" role="status">
      <p>{participant.fullName} needs to accept the academy terms before booking.</p>
      <a className="terms-block-action" href={acceptTermsHref}>
        Review and accept
      </a>
    </div>
  );
...
export function PendingTermsBanner({ studentId }: Readonly<{ studentId: string }>) {
  ...
  const others = status.participants.filter(
    (participant) => participant.studentId !== studentId && pending(participant),
  );
  if (others.length === 0) return null;
  return (
    <p className="calendar-trial-band" role="status">
      {others.map((participant) => participant.fullName).join(", ")} still need to accept the
      academy terms.{" "}
      {session?.role === "teenStudent" ? null : <a href={acceptTermsHref}>Review and accept</a>}
    </p>
  );
}
